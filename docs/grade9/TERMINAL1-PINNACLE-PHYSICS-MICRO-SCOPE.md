...
- `CAP-VEC-DOT-PRODUCT` for angle, perpendicularity and projection;
- `CAP-VEC-CROSS-PRODUCT` for ordered axial direction and unit-normal questions.

These remain external question-demand extensions until school-issued evidence confirms Pinnacle micro-scope. Triple products, coplanarity determinants and vector calculus remain outside the first slice.
